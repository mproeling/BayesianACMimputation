...
%          results.imputation = imputation outcomes
...
impmatrix = zeros(ndraw-nomit, 6);
%ximpute = [1.0000, -1.2766, 0.0400]; % x values of the guy that has to be imputed with y = 0
%yimpute = 0;
ximpute = [1.0000, -0.6131, -1.0305]; % x values of the guy that has to be imputed with y = 1 
yimpute = 1;
for i=1:ndraw; %  Start the sampling
          ...
          if yimpute == 0
              xstarimp = matmul(ximpute,sqrt(1));
              lpimp = xstarimp*bhat; % calculate the mean for the truncated (XtB)
              yimptrue = normrt_rnd(lpimp(1,1),1,0);
              yimpfalse = normlt_rnd(lpimp(1,1),1,0);  
          else
              xstarimp = matmul(ximpute,sqrt(1));
              lpimp = xstarimp*bhat; % calculate the mean for the truncated (XtB)
              yimptrue = normlt_rnd(lpimp(1,1),1,0);
              yimpfalse = normrt_rnd(lpimp(1,1),1,0); 
          end
